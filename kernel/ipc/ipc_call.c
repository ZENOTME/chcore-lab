...
u64 sys_ipc_call(u32 conn_cap, ipc_msg_t * ipc_msg)
{
	...
	if(ipc_msg->cap_slot_number>0){
		for(int i=0;i<ipc_msg->cap_slot_number;i++){
			u64* cap_addr=(u64 *) ((char *)ipc_msg + ipc_msg->cap_slots_offset) + i;
			u64 new_cap=cap_copy(current_process,conn->target->process,*cap_addr,0,0);
			copy_to_user((char*)cap_addr,(char*)&new_cap,sizeof(u64));
		}
	}
